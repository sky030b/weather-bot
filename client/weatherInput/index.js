const API_URL = 'https://opendata.cwa.gov.tw/api/v1/rest/datastore/F-D0047-089';
const API_TOKEN = '';
const LOCATION_NAME = '臺北市';
const ELEMENT_NAME = 'WeatherDescription';
const taiwanOffset = 8 * 60 * 60 * 1000;
const oneDayOffset = 24 * 60 * 60 * 1000;

let weatherData;
async function fetchWeatherData(locationName = LOCATION_NAME) {
  try {
...
async function init() {
  await fetchWeatherData(LOCATION_NAME);
}
...
weatherForm.addEventListener("submit", async (e) => {
  e.preventDefault();
  alert(await analyzeMessageReturnWeather(weatherInput.value));
})
